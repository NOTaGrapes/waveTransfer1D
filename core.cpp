...
void core::computeAndMoveLayer(char* charName) {
	...
	if (stricmp(charName, "lv") == 0) {
		...
		layerUpper_[0] = layerLower_[0];
		for (int i = 1; i != 200; i++) {
			layerUpper_[i] = layerLower_[i] - lambda_ * (layerLower_[i+1] - layerLower_[i - 1])+lambda_*lambda_*2*(layerLower_[i+1] - 2*layerLower_[i]+layerLower_[i-1]);
		}
		layerUpper_[200] = layerLower_[200];
		// click
		iterationClock_++;
	...
	if (stricmp(charName, "mk") == 0) {
		...
		for (int i = 1; i != 200; i++) {
			layerMiddle_[i] =
				layerLower_[i] - lambda_ *
				(layerLower_[i + 1] - layerLower_[i]);
			layerUpper_[i] =
				(layerLower_[i] + layerMiddle_[i] - lambda_ * (layerMiddle_[i] -
				layerMiddle_[i - 1])) / 2;
		}
		layerUpper_[200] = layerLower_[200];
